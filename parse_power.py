...
acc = argv[8]
updateCteMode = argv[9]
paramToIter = argv[10]
increasePerHeight = argv[11]
useMappedComp = argv[12]
getPwrDefault = int(argv[13])
...
with open("%s/%s" % (folder_reports, name_power), "r") as f_power, open("%s/%s" % (folder_reports, name_area), "r") as f_area, open("%s/%s" % (folder_reports, name_nand2), "r") as f_nand2, open("%s/%s" % (folder_reports, name_timing), "r") as f_timing, open("%s/synthesis_results.csv" % (folder_results), "a") as f_out:
	...
	print(",".join([dataset+concStrPwr, classifier, bitsPrecision, maxDepth, numTrees, str(period), str(leak), str(dyn), str(total), str(cells), str(cell_area), str(total_area), str(nand2_eq_total), str(slack), acc, updateCteMode, str(paramToIter), str(increasePerHeight), str(useMappedComp)]), file=f_out)
